...
if [scp ./$FILE $SERVER] then
	...
	if [ssh $SERVER "cat $FILE >> MASTER$FILE; rm $FILE"]then #append the file to the master list
		...
		echo "$? Error printing Data" >> error.log #hopefully this does not happen but generates a very basic error log guves return status though, so it might not be that helpful.
